...
```elixir
@spec map_equal(map(), map()) :: boolean()
```
...
```elixir
@spec map_put_new_lazy(map(), key(), (() -> value())) :: map()
```
...
```elixir
@spec map_values(map()) :: list(value())
```
...
```elixir
@spec pop_lazy(map(), key(), (() -> value())) :: value()
```
...
```elixir
@spec reject(map(), (key(), value()) -> boolean()) :: map()
```
