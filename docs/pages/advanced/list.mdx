...
```elixir
@spec duplicate(list(), 0) :: list()
@spec duplicate([], limit) :: list() when limit: :infinity | pos_integer()
@spec duplicate([...], limit) :: list() when limit: :infinity | pos_integer()
```
...
```elixir
@spec foldl(list(), function(), any()) :: any()
```
...
```elixir
@spec foldr(list(), function(), any()) :: any()
```
...
```elixir
@spec key_member(map(), key()) :: boolean()
```
...
```elixir
@spec update_at(list(), integer(), any()) :: list()
```
